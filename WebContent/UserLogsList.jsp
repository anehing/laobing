...
<center>
<s:form action="queryLogs" method="post" id="form1" >
...
<table class="dataTable" width="97%" border="0" align="center" cellpadding="0" cellspacing="1"  bgcolor="#93b5d1">
	...
	<s:textfield name="userlog.userName" id="username"></s:textfield></td>
		<td width="10%" align="center" bgcolor="#F3F8FE" class="dataTdText">操作日期</td>
	<td width="25%" align="left" bgcolor="#FFFFFF">
	<s:textfield id="userlog.optdate" name="userlog.optdate"  style="border:1px solid #999;" onclick="fPopCalendar(event,this,this)" onfocus="this.select()" readonly="readonly" /></td>
	...
	<tr>
	<td width="10%" align="center" bgcolor="#F3F8FE" class="dataTdText">操作类型</td>
	<td width="25%" align="left" bgcolor="#FFFFFF">
	 <s:select list="#{0:'删除',1:'添加',2:'修改',3:'查询',4:'登录',5:'导出',6:'导入',7:'退出',8:'执行定时'}" name ="userlog.operatype" listKey="key" listValue="value" headerKey="9" headerValue="请选择" ></s:select> 
		...
		<div class="data0">
		   <div class="data1">
				<table class="dataTable" width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td class="topTd" align="center"  width="13%">序号</td>
						<td class="topTd" align="center" width="18%">用户名称</td>
						<td class="topTd" align="center"  width="30%">操作日期</td>
						<td class="topTd" align="center"  width="18%">操作类型</td>
					...
					<s:if test="page.data.size > 0">
						<s:iterator value="page.data" var="t" status="s">
							<tr>
								...
								<td class="dataTd" align="center" >
								 <s:if test="#t.operatype ==0"> 删除</s:if> 
								 <s:if test="#t.operatype ==1"> 添加</s:if> 
											 <s:if test="#t.operatype ==2">修改</s:if>
											 <s:if test="#t.operatype ==3">查询</s:if>
											 <s:if test="#t.operatype ==4">登录</s:if>
											  <s:if test="#t.operatype ==5">导出</s:if>
											  <s:if test="#t.operatype ==6">导入</s:if>
											  <s:if test="#t.operatype ==7">退出</s:if>
											  <s:if test="#t.operatype ==8">执行定时</s:if>
											 <s:else></s:else> 
								</td>
								<td class="dataTd" align="center" >
									<a href="queryUserLogsbyId.action?totype=3&id=${t.id}">详细</a>
								</td>
							</tr>
						</s:iterator>
					</s:if>
